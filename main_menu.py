...
class MainMenuScene (Scene):
	def setup(self):
		...
		self.play_button = SpriteNode('./assets/sprites/play.PNG', size = (self.size[0]/3.84, self.size[1]/15.36), position = (self.size[0]/1.25,self.size[1]/2), parent = self.main_menu_node)
		
		self.settings_button = SpriteNode('./assets/sprites/settings.PNG', size = (self.size[0]/3.84, self.size[1]/15.36), position = (self.size[0]/1.25,self.size[1]/2.75), parent = self.main_menu_node)
		
		self.credits_button = SpriteNode('./assets/sprites/credits.PNG', size = (self.size[0]/3.84, self.size[1]/15.36), position = (self.size[0]/1.25,self.size[1]/4.50), parent = self.main_menu_node)
	...
	def touch_began(self, touch):
		...
		if self.settings_button.frame.contains_point(touch.location) and self.main_menu_node.alpha == 1:
			
			self.main_menu_node.run_action(Action.fade_to(0,1))
			
			self.background.run_action(Action.move_to(0, 0, 1))
			
			self.settings_menu_node.run_action(Action.fade_to(1,1))
		...
		if self.back_to_main_menu_button_two.frame.contains_point(touch.location) and self.settings_menu_node.alpha == 1:
			
			self.settings_menu_node.run_action(Action.fade_to(0,1))
			self.main_menu_node.run_action(Action.fade_to(1,1))
			self.background.run_action(Action.move_to(self.size[0],0,1))
